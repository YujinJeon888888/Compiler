...
%token TLPAREN TRPAREN TCOMMA TLBRACE TLBRACE TSEMI TASSIGN 
%token TLSQUARE TRSQUARE TGREAT TLESS TPLUS TMINUS TMUL TDIV TMOD TNOT TRBRACE
...
%token TOR TAND TEQUAL TNOTEQU TGREATE TLESSE TINC TDEC TSTRING TINTNUM TFLOATNUM TFLOAT TERROR TDOT TCHAR
...
opt_expression 	: expression					{semantic(47);}
		 	|						{semantic(48);};
if_st    : TIF TLPAREN expression TRPAREN statement TEIF TSEMI		{ semantic(49); }
        | TIF TLPAREN expression TRPAREN statement TELSE statement TEIF TSEMI	{ semantic(50); };
