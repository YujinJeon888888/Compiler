...
/*지속적으로 업뎃되어야하므로 지역변수x 전역변수o */
int index_start = 0;
int sym_table_index = 0;

/**
 * @brief  심볼 테이블을 처리하는 함수
 * @details hast 계산 &  string pool저장 > 해시테이블 저장 > symbol table에 추가 > 해시 테이블에 링크시키기
 * @param identifier: 읽은 yytext
 * @return  -1: 이미 존재하는 경우, 0: 새로 추가된 경우
 */
int process_sym_table(char* identifier) {
    int hash_value = 0;
    bool flag_undefined = false;
 
	//인덱스 시작위치에서 저장될 수 있게 +index_start에 저장
    //index start: 계속 가지고 있어야하는값 => 따라서 로컬변수x 전역변수o
    strcpy(str_pool + index_start, identifier);

    //해시코드 계산
    hash_value = divisionMethod(str_pool + index_start, HASH_TABLE_SIZE);
    
    //해시테이블 look up 후 새로운거면 해시테이블에 저장
